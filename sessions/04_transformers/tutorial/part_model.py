"""
Self-contained Particle Transformer (ParT) with attention-weight capture.

Vendored and lightly adapted from
  https://github.com/aaronw5/Interpreting-Particle-Transformers (AttentionGraphs.ipynb),
which itself follows the official model in weaver-core
  https://github.com/hqucms/weaver-core/blob/main/weaver/nn/model/ParticleTransformer.py

Adaptations:
  * removed the `weaver` dependency (the logger is stubbed) so this runs with a
    plain PyTorch install -- no weaver-core needed;
  * each transformer Block stores its attention weights (Block.interaction,
    shape (batch, num_heads, seq_len, seq_len)) captured directly from the
    forward pass, exposed via ParticleTransformer.getAttention().
"""
...
import logging
_logger = logging.getLogger('ParT')  # standalone stub; avoids the weaver dependency
    ...
    def forward(self, x, x_cls=None, padding_mask=None, attn_mask=None):
        ...
        if x_cls is not None:
            ...
            x, attn_weights = self.attn(
                ...
                need_weights=True, average_attn_weights=False)  # x: (seq_len, batch, embed_dim)
            self.interaction = attn_weights  # (batch, num_heads, seq_len, seq_len)
